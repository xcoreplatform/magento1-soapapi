 ...
 */
class Dealer4dealer_Xcore_Block_Adminhtml_System_Config_Field_Renderer_Salesflatinvoicecolumn
...
WHERE
  `table_schema` = '{$dbname}'
   AND `table_name` = '{$resource->getTableName('sales/invoice')}'
ORDER BY
  `table_name`, `ordinal_position`
        ");

        foreach ($results as $row) {
            $this->addOption($row['column_name'], "{$row['column_name']} ({$row['data_type']})");
        }
